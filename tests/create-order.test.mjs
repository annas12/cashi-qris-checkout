...
const tests = [
  ...
  [
    "Produk NF-1 menghasilkan harga backend 5000",
    async () => {
      const mock = createSuccessFetch((url, init) => {
      ...
      assert.equal(data.base_amount, 5000);
      assert.equal(db.orders[0].base_amount, 5000);
    }
  ],
  [
  ...
  [
    ...
    async () => {
      ...
      assert.equal(db.orders[0].payment_amount, 5023);
